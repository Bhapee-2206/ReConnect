      ...
      <main className="pt-32 pb-20 px-6 md:px-12 max-w-7xl mx-auto">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="relative">
            {/* Redesigned Visual representing Alumni Connectivity */}
            <div className="relative z-10 bg-white rounded-[3rem] aspect-square overflow-hidden shadow-2xl border border-surface-container rotate-3 scale-95 md:scale-100 p-8">
                <div className="h-full w-full rounded-2xl bg-slate-50 relative overflow-hidden flex flex-col items-center justify-center border border-slate-200">
                    ...
                    <div className="space-y-6 text-center z-10">
                        <div className="flex justify-center -space-x-4">
                            {[1,2,3].map(i => (
                                <div key={i} className="w-16 h-16 rounded-full border-4 border-white bg-indigo-100 flex items-center justify-center text-indigo-600 shadow-xl">
                                    <span className="material-symbols-outlined">person</span>
                                </div>
                            ))}
                        </div>
                        <div className="space-y-2">
                            <p className="text-2xl font-black text-on-surface tracking-tighter">Verified Network</p>
                            <div className="flex items-center justify-center gap-2">
                                <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
                                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-outline">Live Connection</span>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            {/* Floating Cards - Re-themed for Careers/Mentorship */}
            <div className="absolute -top-10 -right-10 bg-white p-6 rounded-3xl shadow-2xl animate-float hidden md:block z-20 border border-surface-container">
                <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-2xl bg-indigo-50 text-indigo-600 flex items-center justify-center">
                        <span className="material-symbols-outlined text-2xl">work_history</span>
                    </div>
                    <div>
                        <p className="text-[10px] font-black uppercase text-outline mb-0.5 leading-none">Career Pulse</p>
                        <p className="text-xl font-black text-on-surface">1.2k+ Placements</p>
                    </div>
                </div>
            </div>
            <div className="absolute -bottom-6 -left-6 bg-white p-5 rounded-2xl shadow-xl hidden md:block z-20 border border-surface-container animate-float-delayed">
                <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-amber-50 text-amber-600 flex items-center justify-center">
                        <span className="material-symbols-outlined text-lg">workspace_premium</span>
                    </div>
                    <p className="text-xs font-bold text-on-surface">98% Mentor Match</p>
                </div>
            </div>
          </div>
        </div>
